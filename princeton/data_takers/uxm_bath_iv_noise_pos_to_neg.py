...
for bg in bias_groups: # RS: This seems redundant?
...
for bias_gp in bias_groups:
    ...
    row['band'] = 'all'
    row['bias_voltage'] = 'IV 19 to -15' # corrected
    row['type'] = 'IV'
    print(f'Taking IV on bias line {bias_gp}, all band')
    
    # Settings inspired by Daniel's Lp1r1 data taking.   
    row['data_path'] = det_ops.take_iv(
        S,
        cfg,
        bias_groups = [bias_gp],
        wait_time=0.05, # how long to wait at each step. was 0.01; DD used 0.11 for one set; I'm doing 0.5 for more speed
        bias_high=19, #  I like more normal branch, 19 instead of 15.
        overbias_wait=2, # wasn't included
        bias_low=-15,  # was 0
        bias_step = 0.025,
        overbias_voltage=12, # was 19
        cool_wait=30,
    ...
    time.sleep(150) # was 30, but I increased the time at each step
