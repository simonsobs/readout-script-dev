...
def read_loss_in(config_dir, in_fname, f_GHz):
    ...
    c = pd.read_csv(os.path.join(config_dir,'RF_Chains',in_fname), delimiter=',')
    ...
    for i in range(len(compin)):
        if coax_YNin[i] == 'Y':
            t = readcoax(os.path.join(config_dir,'Coaxes',coax_typein[i]+'.csv'),f_GHz)
            lossin[i] = (t[0]*Tavgin[i]+t[1])*coax_lin[i]
    return lossin, Tavgin

def read_loss_out(config_dir, out_fname, f_GHz):
    ...
    c = pd.read_csv(os.path.join(config_dir,'RF_Chains',out_fname),delimiter=',')
    d = np.transpose(c.values)
    compout = d[0,:]
    lossout = d[1,:]
    Tavgout = (d[2,:]+d[3,:])/2
    ...
    for i in range(len(compout)):
        if ((lossout[i] < 0) and (d[7,i] != np.nan)):
            ampfpath = glob.glob(os.path.join(config_dir,'Amps','*'+str(d[7,i])+'*'))[0]
            ampdf = pd.read_csv(ampfpath,delimiter=',')
            ampdarr = np.transpose(ampdf.values)
            lossout[i] = -np.interp(f_GHz,ampdarr[2,:],ampdarr[3,:])
            LNA_TN.append(np.interp(f_GHz,ampdarr[0,:],ampdarr[1,:]))
        if coax_YNout[i] == 'Y':
            t = readcoax(os.path.join(config_dir,'Coaxes',coax_typeout[i]+'.csv'),f_GHz)
            lossout[i] = (t[0]*Tavgout[i]+t[1])*coax_lout[i]
    return lossout, Tavgout, LNA_TN
...
def Calc_All_Noise(dBc,Qr,Qc,f0,dfdI,P_feed,loss_in,temp_in,temp_out,loss_out,
                   Pin_300K,LNA_TN,Ntones, p=False):
    """
    Parameters:
    dBc (float): Input phase noise (positive number) from DAC chain (no default but typical for SMuRF is 101 dBc/Hz)
    Qr (float): Resonator quality factor.
