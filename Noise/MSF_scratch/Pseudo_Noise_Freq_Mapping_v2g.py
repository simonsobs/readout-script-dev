...
df_mmb = pd.read_pickle('/home/arnoldlabws3/Scratch/MSF/Personal_Laptop_Docs/SO/MMBv2g/20190926-mccarrick_mmbv2g_resonator_params.pkl')
...
for fs in f0s_reas:
	plt.plot([fs,fs],[np.min((amps/noise_bw_step)/kb)-100,200],'c-')
plt.ylim([np.min((amps/noise_bw_step)/kb)-10,150])
plt.plot(freqs_binned[0:-1],(amps/noise_bw_step)/kb,'o')
T_PNF = 144.26802447176655
plt.plot([1e9,9e9],[T_PNF,T_PNF],'r-',label = 'PNF White Noise Assumption for Buddy Review Assuming Every Dip is Equal Depth Set equal to the median of MMBv2')     
tot_noise = np.sum(amps)/(np.max(nl_freqs_sort)-np.min(nl_freqs_sort))/kb                          
plt.plot([1e9,9e9],[tot_noise,tot_noise],'k--',label = '(Sum of All Pseudo Tone Amps from MMBv2p Calc)/(Pseudo Tone Total Bandwidth from MMBv2p Calc)') 
plt.xlim([2e9,8e9]) 
